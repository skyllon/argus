...
- Separer les REGLES des FAITS MESURES (nouveau v5.1). Un document de regles donne son autorite a tout ce qu'il contient : un fait mesure qui y figure (quota, tarif, plafond de concurrence, plancher, gain annonce) perit sans bruit, garde l'apparence d'une valeur verifiee, et continue de fonder des decisions longtemps apres etre devenu faux. Pire, un chiffre ecrit SUPPRIME le reflexe de le verifier.
- Test d'appartenance avant d'ecrire un nombre dans un document de regles (nouveau v5.1) : si ce nombre changeait demain, est-ce que quelque chose me le dirait ? Si oui c'est une politique (un seuil decide, une norme d'ergonomie) et elle reste. Si non c'est une observation : elle va dans un registre separe, avec sa date de mesure, sa duree de validite et la commande qui la re-mesure.
- Aucun chiffre de performance, de cout ou de capacite ne s'affirme de memoire (nouveau v5.1) : il se lit dans le registre, et s'il a depasse sa validite il se re-mesure ou ne s'emploie pas. Un fait date se perime TOUT SEUL a la lecture, puisque la date du jour est connue a chaque session : c'est ce qui rend le dispositif robuste sans aucune infrastructure. Une sonde periodique qui re-verifie ce qui est verifiable localement n'est qu'un second filet.
